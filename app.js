...
(function (global) {
	...
	async function loadURL(url) {
		...
		if (xmlScript) {
			...
			_function = new Function(["global", "document", "content", "contentSize", "load", "uuid"], mq + ";;;;;");
		...
		for (let k in elements) {
			...
			for (let am = 0; am < 90; am++) {
				...
				if (item.name.startsWith("webstyle:")) {
					...
					listeners[k].push([name, (event) => {
						event.preventDefault();
						let man = new Function(["global", "event", "$", "sendServer", "loadURL", "load", "uuid"], item.value);
						man(this, event, (e) => document.getElementById(e), sendServer, validateLink, load, generateUUID);
					}]);
